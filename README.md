...
1. `GET /api/proof-map` — choose whether the quant or advisory lane is the right first proof
2. `GET /api/runtime/brief` — confirm the shared platform boundary
3. `GET /api/quant/research-pack` — read the strongest quant proof lane
4. `GET /api/advisory/review-pack` — read the strongest advisory proof lane
...
| `GET /api/proof-map` | Front-door proof lane chooser |
