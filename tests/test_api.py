...
def test_runtime_brief_structure() -> None:
    ...
    assert body["links"]["proof_map"] == "/api/proof-map"
    assert body["reviewer_fast_path"][0] == "/api/proof-map"
    assert "quant" in body["domains"]
    assert "advisory" in body["domains"]


def test_proof_map_structure() -> None:
    resp = client.get("/api/proof-map")
    assert resp.status_code == 200
    body = resp.json()
    assert body["schema"] == "finance-review-proof-map-v1"
    assert body["reviewer_fast_path"][0] == "/api/proof-map"
    assert body["route_groups"]["quant"][-1] == "/api/quant/research-pack"
...
def test_client_review_rejects_invalid_risk_profile() -> None:
    ...
    with pytest.raises(ValidationError):
        ClientReview(
            ...
            meeting_type="annual",
            risk_profile=cast(Any, "yolo"),
            liquidity_need=cast(Any, "medium"),
            current_focus=["test"],
        )
...
def test_client_id_strips_whitespace() -> None:
    ...
    from app.domains.advisory.schemas import ClientReview, LiquidityNeed, RiskProfile
